...
6. **Annotate.** Agents can flag emitted entities as wrong, stale, or worth attention via the notes surface ([issue #186](https://github.com/yaad-index/yaad-index/issues/186), shipped 2026-05-23). Notes carry an optional `kind=annotation` flag so agent-feedback can be filtered cleanly from regular operator notes.
