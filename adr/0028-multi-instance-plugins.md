...
3. **Cut 3 — URL dispatch via `instance_routing`.** Wire the `instance_routing` block in `--init` capabilities through the plugin loader; implement glob-match strategy per §3 with first-match-wins + first-declared fallback + overlap-warning. Update the existing URL routing path in `internal/api/` to consult the per-instance routing table. Pair with a `instance_routing`-bearing example plugin (yaad-github fits naturally per ADR-0026 §7).
