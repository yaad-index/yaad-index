...
`gap_state[<gap>].source` is stamped `operator` — this endpoint records the gap as operator-curated regardless of trigger-mode, so an agent-trigger write to an operator-strategy gap stamps `source: operator` (the value's origin is operator input). The precise per-write audit trail is the separate fill-provenance entry, which records the calling subject (the agent that executed the write).
