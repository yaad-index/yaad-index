...
Per [ADR-0016](../../adr/0016-canonical-kind-defaults.md), the five operator-strategy gaps on `boardgame` are declared at the plugin layer (Layer 2 of the gap-merge chain). `fill_strategy: operator` annotates the value's source — operator input — so no `clean_content` ingest signal fills them automatically; the agent surfaces them out-of-band and writes the operator's confirmed value via `POST /v1/entities/{id}/fill` (the unified endpoint per [ADR-0029](../../adr/0029-unified-fill-surface.md); the old `/operator-fill` URL is `410 gone`). Per the ADR-0029 §3 amendment a bare agent token may perform this write — the strategy governs the value's source, not write-permission. The operator can override any of these via `canonical_kinds.boardgame.gaps.<field>` in `yaad-index.yaml` — the operator layer wins.
