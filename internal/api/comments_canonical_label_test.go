...
// operator-fill" to "first operator action" — comments authored by
...
// add_comment auto-materialize tests. Mirrors
...
// TestAddComment_AutoMaterialize_AgentOnBehalf pins the headline
// contract: an agent JWT with operator-claim populated can
// post a comment to a canonical-label thin row, and the daemon
// auto-creates the vault file at `{ROOT}/ct/<kind>/<slug>.md`
// with the comment as the first attached content.
func TestAddComment_AutoMaterialize_AgentOnBehalf(t *testing.T) {
	t.Parallel()
	...
	rec := ugcReq(t, h, http.MethodPost, "/v1/entities/"+id+"/comments", agentTok,
	...
	assert.Equal(t, "operator's note via agent", got.Comment.Text)
	assert.Equal(t, "the implementer", got.Comment.Author, "author stamps the agent (subject)")
	assert.Equal(t, "alice", got.Comment.Operator, "operator stamps the human (claim.Operator)")
	...
	require.NoError(t, err, "vault file must be readable post-comment")
	require.Len(t, ve.Comments, 1, "comment landed in vault")
	assert.Equal(t, "operator's note via agent", ve.Comments[0].Text)
	assert.Equal(t, "the implementer", ve.Comments[0].Author)
	assert.Equal(t, "alice", ve.Comments[0].Operator)
}

// TestAddComment_AutoMaterialize_OperatorDirect: the operator-direct
// path (Subject == Operator) also auto-materializes — the gate
// accepts both shapes uniformly.
func TestAddComment_AutoMaterialize_OperatorDirect(t *testing.T) {
	t.Parallel()
	h, st, root, signer := newCommentsCanonicalLabelFixture(t)
	...
	rec := ugcReq(t, h, http.MethodPost, "/v1/entities/"+id+"/comments", tok,
	...
	ve, err := r.ReadByID("boardgame", id)
	require.NoError(t, err)
	require.Len(t, ve.Comments, 1)
	assert.Equal(t, "alice", ve.Comments[0].Author)
	assert.Equal(t, "alice", ve.Comments[0].Operator)
}

// TestAddComment_NoAutoMaterialize_NoRow: per the operator's scope
// tightening, comments do NOT create entities from nothing —
// even with operator authority + canonical-label-shaped id,
// when the entity row doesn't exist the request returns 404.
// Operator-fill is the deliberate-create path; comments are
// casual and need an existing entity to attach to (otherwise
// dangling comments accumulate on entities that don't exist).
func TestAddComment_NoAutoMaterialize_NoRow(t *testing.T) {
	t.Parallel()
	h, st, _, signer := newCommentsCanonicalLabelFixture(t)
	const id = "boardgame:operator-invented-game"

	// No seed at all — agent attempts to comment on a canonical
...
// TestAddComment_NoAutoMaterialize_NonCanonicalLabelID: an id that
...
func TestAddComment_NoAutoMaterialize_NonCanonicalLabelID(t *testing.T) {
	t.Parallel()
...
// TestAddComment_ExistingVaultFile_NoAutoMaterializePath: when the
// vault file already exists, the auto-materialize path doesn't
// fire — the comment appends via the existing flow. Pinned so a
// regression in the autoMaterialize flag doesn't accidentally
// re-materialize an existing entity (which would overwrite Data).
func TestAddComment_ExistingVaultFile_NoAutoMaterializePath(t *testing.T) {
	t.Parallel()
	h, st, root, signer := newCommentsCanonicalLabelFixture(t)
