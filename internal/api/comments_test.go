...
const commentsTestEntityID = "boardgame:comment-test"

// newCommentsFixture builds a vault-wired handler + a seeded entity
// (DB row + vault file) ready to accept POST /v1/entities/{id}/comments
// calls. Mirrors newFillFixture's shape.
func newCommentsFixture(t *testing.T) (http.Handler, store.Store, string) {
	t.Helper()
...
// ADR-0008). No comments yet; the test exercises the append path.
func seedCommentsEntity(t *testing.T, st store.Store, vaultRoot, id, kind string) {
	...
	require.NoError(t, w.Write(&vault.Entity{
		...
		Kind: kind,
		Plugin: "test-fixture",
		Data: map[string]any{"id": id, "title": "Comment Test Game"},
	}))
}
...
func TestComments_HappyPath(t *testing.T) {
	...
	assert.Equal(t, "alice", got.Comment.Author)
	assert.Equal(t, "Played this last night, thoroughly enjoyed the canal phase.", got.Comment.Text)
	_, err := time.Parse(time.RFC3339, got.Comment.Date)
	require.NoError(t, err, "comment.date is RFC3339")

	// Vault file has the comment in frontmatter (canonical) + body
	// `## Comments` section (regenerated mirror).
	v := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	require.Len(t, v.Comments, 1)
	assert.Equal(t, "alice", v.Comments[0].Author)
	assert.Equal(t, "Played this last night, thoroughly enjoyed the canal phase.", v.Comments[0].Text)
	assert.False(t, v.Comments[0].Date.IsZero(), "server-stamped date set")
}
...
func TestComments_AccumulateAcrossCalls(t *testing.T) {
	...
	v := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	require.Len(t, v.Comments, 3, "three appends → three frontmatter rows")
	for i := 1; i < len(v.Comments); i++ {
		assert.False(t, v.Comments[i].Date.Before(v.Comments[i-1].Date),
			"comments[%d].date must be ≥ comments[%d].date (chronological)", i, i-1)
	}
}
...
func TestComments_TrimsLeadingAndTrailingWhitespace(t *testing.T) {
	...
	// The cold-reviewer's a prior PR review note: comment text with leading newlines
	...
	var got commentsResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&got))
	assert.Equal(t, "hello world", got.Comment.Text, "text trimmed on input")
	assert.Equal(t, "alice", got.Comment.Author, "author trimmed on input")

	v := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	require.Len(t, v.Comments, 1)
	assert.Equal(t, "hello world", v.Comments[0].Text)
	assert.Equal(t, "alice", v.Comments[0].Author)
}
...
func TestComments_RejectsMalformedJSON(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost,
		"/v1/entities/"+commentsTestEntityID+"/comments", strings.NewReader(`{`))
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestComments_RejectsUnknownFields(t *testing.T) {
	...
	h, _, _ := newCommentsFixture(t)
	req := httptest.NewRequest(http.MethodPost,
		"/v1/entities/"+commentsTestEntityID+"/comments",
...
// dated `## Comments` block into the body in Obsidian, followed by
...
func TestComments_HandEditPicksUpInBodyThenAPIAppendKeepsBoth(t *testing.T) {
	...
	require.Len(t, got.Comments, 2, "both hand-edit + API-append survive")

	// Order is by the vault.mergeComments stable sort by Date ASC.
	// Hand-edit (older) first, API-append (now) second.
	assert.Equal(t, "hand-edited body comment", got.Comments[0].Text)
	assert.Empty(t, got.Comments[0].Author)
	assert.Equal(t, "API-append after the hand-edit", got.Comments[1].Text)
...
func TestComments_StaleWriterOverwritesBodyHandEdit(t *testing.T) {
	...
	// First API-append: vault file now has comment A in frontmatter +
	// body section.
	rec := postComments(t, h, commentsTestEntityID, map[string]any{
		"text": "comment A from API",
		"author": "alice",
	})
	require.Equal(t, http.StatusCreated, rec.Code)

	// In-memory state from before the API call (i.e., no comments).
	stale := &vault.Entity{
		ID: commentsTestEntityID,
		Kind: "boardgame",
		Plugin: "test-fixture",
		Data: map[string]any{"id": commentsTestEntityID, "title": "Comment Test Game"},
	}
	...
	// Comment A is gone — the stale writer didn't merge body content.
	got := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	assert.Empty(t, got.Comments, "stale writer overwrites: documented staleness window")
}

// TestComments_FTSFindsCommentText locks the vaultEntityDataForDB
// projection extension: the LIKE-on-data search finds comment text
// because we fold concatenated comment text into `data["comments_text"]`
// on each UpsertEntity.
func TestComments_FTSFindsCommentText(t *testing.T) {
	t.Parallel()

	h, _, _ := newCommentsFixture(t)
	rec := postComments(t, h, commentsTestEntityID, map[string]any{
		"text": "this comment contains the word abracadabra",
		"author": "alice",
	})
	require.Equal(t, http.StatusCreated, rec.Code)

	// q=abracadabra (a word that appears nowhere in the seeded
	// fixture's data) should hit because comments_text is in data.
	...
	require.NotEmpty(t, resp.Results, "search should find the entity by comment text")
	var found bool
...
// flow through reindex" claim. Hand-edit a body comment, run
// reindex, query → entity has the comment. End-to-end vault →
// reindex → DB property.
func TestComments_ReindexRoundTripsHandEdit(t *testing.T) {
	t.Parallel()

	h, st, root := newCommentsFixture(t)
	...
	// Hand-edit: a fresh body-only comment that the API never saw.
	w, err := vault.NewWriter(root)
	require.NoError(t, err)
	v := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	v.Comments = append(v.Comments, vault.Comment{
	...
	// Search finds the hand-edited text via the comments_text
	...
	// reindex, the entity's comments are in the vault (this test
	// directly asserts the vault state survived round-trip).
	v2 := readVaultByID(t, root, "boardgame", commentsTestEntityID)
	require.Len(t, v2.Comments, 2, "API + hand-edit both present")
	gotTexts := make([]string, 0, len(v2.Comments))
	for _, c := range v2.Comments {
