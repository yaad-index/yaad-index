...
### `add_comment(entity_id, text, author?)`

Append a comment to an existing entity. Server stamps the date (UTC), the agent identity (`author`, from your JWT subject), and the human resource owner (`operator`, from your JWT pair-claim). Returns `{ok, comment, entity}` where `comment` is the just-appended entry and `entity` is the merged-comment-included fresh copy of the entity (so you don't need a follow-up `get_entity`).
...
- **Errors pass through structured.** Unlike most tools, `add_comment` does NOT throw on auth errors. A 4xx from yaad-index returns `{ok: false, error, message}` directly so you can branch on `error === "author_mismatch"` / `"missing_authorization"` / etc. Successful calls return `{ok: true, comment, entity}`. Read `ok` first.
...
**When to use.** Reach for `add_comment` when you want to leave human-readable context on an entity that survives reindex (vault-frontmatter-mirrored). Comments aren't gap fills — gap fills carry structured derivable data with a defined shape; comments are free-form prose for "I noticed X" / "this looks wrong" / "operator should verify". Don't use comments to encode structured data the gap mechanism could carry.
...
| Append a comment to an entity (free-form prose, not gap data) | `add_comment(entity_id, text)` |
...
A `complete` or `needs_fill` response means the platform is up. Connection errors mean check `YAAD_INDEX_URL` and that the Docker pilot is running.
