...
> ⚠️ **Pre-release — the contract may change between sessions.** Tool names, response shapes, gap composition, and the entity wire format may change without notice or a migration path until a stable release is cut. Re-read the response shape from the tool's actual output, not from this doc, when the two disagree.

> **🤖 Built with AI.** yaad-index was designed and written by AI agents in collaboration with the project lead; the design rationale lives in the repository's [Architecture Decision Records](../adr/).
...
- **UGC rename (`rename_user_content`)** — renaming an entity registers the prior id's bare slug as an alias to the new id, so references to the old `user-content:<old-slug>` keep resolving after the rename.
...
### `needs_fill(limit?, cursor?, source?, kind?)`
...
- `source` scopes the queue to one plugin namespace — the bit before `/` in the entity's `source` (e.g. `bgg`, `wikipedia`, `github`, `gmail`). Plugin-level, not per-instance: `bgg` matches `bgg/default` and any other `bgg/*` instance.
- `kind` scopes the queue to one entity kind (e.g. `boardgame`, `person`). `source` and `kind` compose with **AND**; omit both for the full queue. Caveat: `total` reflects the `kind` filter but **not** `source` — under a source filter `total` is the kind-filtered queue depth, and the source narrowing applies to the returned entities.

**No auto-pagination, no cross-page aggregation.** `source` / `kind` are applied server-side by the daemon; beyond those, the queue is surfaced verbatim — the agent decides whether to keep paginating, when to stop, and whether to act on each entry (typically by calling `fill(id, fields)`). No client-side aggregation or caching.
...
- **`note_id` (ADR-0015 §Note identity).** Each note carries a stable 8-hex-char `note_id`, stamped server-side at creation and surfaced on `note` (the just-created note's id) and on every entry in `get_entity`'s `notes[]`. It's the handle future edit/delete tools target. Legacy notes written before the id existed surface an empty `note_id` until the next note operation on that entity re-stamps the whole block (then all carry ids).
...
- **Edit / delete by `note_id`** (no threading). Use `edit_note` / `delete_note` (below) to correct or retract a note rather than appending a corrective one.
- **Task entities (`kind: task`)** route the note into the task body's 5-section `notes` section (not the legacy `## Notes` table); the note lands as a single dated, attributed line and carries no `note_id` (#343).
...
### `edit_note(entity_id, note_id, text, field?, kind?)`

Edit an existing note in place, targeted by its `note_id` (from a prior `add_note` response or `get_entity`'s `notes[]`). Replaces `text` (and `field` / `kind` — a note edit is a whole-note replace of the mutable fields, not a patch) and stamps `last_edited_at` alongside the original created date; the `note_id` is stable across edits. Returns `{ok, note, entity}` with the edited note (now carrying `last_edited_at`).

- **Author-gated.** Only the note's author may edit it; anyone else gets 403 `author_mismatch`. (Workflow-authored notes — `author: workflow:<name>` — aren't agent-editable.) Errors pass through structured (`{ok: false, error, message}`).
- 404 `not_found` when the entity or `note_id` doesn't exist. A legacy note that still surfaces an empty `note_id` can't be targeted until a note op on that entity stamps ids (see `add_note`).

### `delete_note(entity_id, note_id)`

Hard-delete a note by its `note_id` (no tombstone — the vault's git history covers audit). Returns `{ok, id, note_id, deleted: true}`.

- **Author-gated** (same rule as `edit_note`): only the author may delete; 403 `author_mismatch` otherwise. 404 `not_found` when the entity or `note_id` doesn't exist (a second delete of the same id 404s).

**Section tools also work on canonical entities (ADR-0031).** The six section tools — `list_user_content_sections`, `get_user_content_section`, `add_user_content_section`, `edit_user_content_section`, `rename_user_content_section`, `delete_user_content_section` — operate on any entity that carries an operator-authored body surface, not just `user-content:` ids. That means a canonical thin-edge (`boardgame:<slug>`, `person:<slug>`, etc.) can carry a free-form markdown body written directly on its page — the daemon marks canonical entities body-editable when it materializes them. Plugin source entities (`bgg:`, `gmail:`, `github:`, `wikipedia:`) are NOT body-editable and return `{error: "invalid_argument", ... "does not accept user-content sections"}`. The first operator to write a section on a canonical entity claims ownership; subsequent edits are operator-keyed exactly like UGC (cross-operator writes → 403 `operator_mismatch`). The entity-level tools (`get_user_content`, `create_user_content`, `delete_user_content`) remain `user-content:`-only.
...
Operator-side fill. Routes per-field operations to the unified endpoint `POST /v1/entities/{id}/fill` (per ADR-0029 — the old `/v1/entities/{id}/operator-fill` URL is `410 gone`; this tool stays as an alias of `fill_field`). Per the ADR-0029 §3 amendment (#521), the strategy gate is **one-directional**: an operator-strategy field accepts an agent-trigger write — the agent surfaces the field to the operator out-of-band, the operator answers, and the agent writes the confirmed value here (provenance stamps the filling agent's subject). `operator_only_field` is no longer emitted. The `operator_delegated` claim (`issue-token --on-behalf-of-operator` per #361) is still required only for the paths that remain operator-trigger-only: ad-hoc writes to non-gap fields, and `defer`.
...
### `create_canonical_entity(kind, slug, data?)`

Create a canonical entity (`<kind>:<slug>`) **directly**, without the indirect paths (plugin ingest, an edge side-effect from another entity, or a UGC frontmatter-edge stub). Use when you want a canonical page — e.g. `person:alex-example` — to exist on its own *before* any source or UGC references it, so you can write notes / frontmatter / a body on it.

- **Edge-side-effect-free** by design: it creates only the entity (vault file + DB row + `entity.created` event), no edges. `data` may seed scalar gap fields (same typed validation + `gap_state` stamping as `fill`), but a `canonical_type` (edge-producing) gap is **rejected** with 400 — seed those with `fill_field` after creation, where the edge materialization is explicit.
- `kind` must be in the operator's `canonical_kinds:` registry (else 400). `slug` must be ADR-0008-shaped — lowercase alphanumeric segments joined by single hyphens (else 400). 409 on slug collision.
- **Any-authenticated**, not operator-only — agents already stub canonicals indirectly via edge side-effects, so the direct path isn't more restricted. The created entity is `ugc: true` (per ADR-0031), so its body is section-editable via the user-content section tools.
- Returns the created entity envelope + the remaining open gaps.
...
### `move_user_content(id, subfolder?)`

Relocate a UGC entity's vault file to a different on-disk subfolder **in place** — no archive → delete → recreate dance. The entity **id stays the same**: the subfolder is purely on-disk organization, not part of the identity, so provenance, edges, and the content-derived `etag` are all preserved by construction. Returns the entity envelope (same shape as `get_user_content`).

- `subfolder` — a single path segment of lowercase alphanumerics and hyphens (e.g. `notes`, `drafts`, `projects`). Empty / omitted moves the file to the flat `user-content/<slug>.md` path.
- Moving to the subfolder the file already lives in is an idempotent no-op (still `200`).
- A malformed subfolder rejects with `400`; an unknown id is `404`.
- Operator-keyed exactly like the section tools — a cross-operator move returns `403 operator_mismatch`.

### `rename_user_content(id, new_title)`

Rename a UGC entity by **retitling** it: the new slug — and thus the new id `user-content:<new-slug>` — is derived from `new_title` server-side, the same way `create_user_content` derives a slug from `title`. Unlike `move_user_content` (which keeps the id and relocates the file), rename **changes the identity**. Returns the renamed entity envelope + a fresh `etag` (the body is unchanged, so the etag is stable across the rename).

- Inbound + outbound **edges, provenance, and the file body are preserved**, re-keyed to the new id in one transaction; the vault file (and its attachment sidecar) is renamed to the new slug in its current location.
- The **old id keeps resolving**: the prior bare slug is registered as an alias to the new id, so existing `user-content:<old-slug>` references (and any inbound edges) still reach the entity.
- A `new_title` that slugifies to the **current** slug is an idempotent no-op (`200`, unchanged).
- `409 conflict` when the new slug collides with an existing entity, vault file, or alias — pick a different title. `404` on unknown id; `403 operator_mismatch` on a cross-operator rename.
...
| Find entities with open gaps (queue browse) | `needs_fill(limit?, cursor?, source?, kind?)` |
| Browse open gaps from one plugin or kind | `needs_fill(source: "bgg", kind: "boardgame")` |
...
| Correct / retract a note you wrote (author-gated, by note_id) | `edit_note(entity_id, note_id, text)` / `delete_note(entity_id, note_id)` |
...
| Move a UGC entity's file to another subfolder (id unchanged) | `move_user_content(id, subfolder: "notes")` |
| Rename a UGC entity (new id from new title; old id stays an alias) | `rename_user_content(id, new_title)` |
